...
% Last Modified by GUIDE v2.5 23-Jan-2017 15:28:13
...
fileID = fopen('status1.txt','r');
formatSpec = '%d';
status1 = fscanf(fileID,formatSpec);
fclose(fileID);
if status1==1
    readfileID = fopen('nums1.txt','r');
    formatSpec = '%f';
    A = fscanf(readfileID,formatSpec);
    assignin('base','A',A);
    assignin('base','pos_target',A);
    fclose(readfileID);    
...
dXYZ=delta(1:3,1)/10;  
dTheta=Jacinv*dXYZ;
dTheta1=rad2deg(dTheta(1,1));
dTheta2=rad2deg(dTheta(2,1));
dTheta3=rad2deg(dTheta(3,1));
j1=j1+dTheta1;
j2=j2+dTheta2;
j3=j3+dTheta3;
angle=[j1;j2;j3];
...
if EucError <10^-2
   ...
   writefileID = fopen('nums2.txt','w');
   fprintf(writefileID,'%f\n',angle);
   fclose(writefileID);
   statusfileID = fopen('status1.txt','w');
   fprintf(statusfileID,'%d\n',0);
   fclose(statusfileID);
end 
