...
int formmax = 180;
int tomin = 750; //750 = 0
int tomax = 2250; //2250 = 180

int target_base_step = 0;
int current_base_step = 0; 
int step_distance = 0;
int joint1_angle = map(90,formmin,formmax,tomin,tomax);
...
void setup() {
  ...
  OCR1A = 311;// = ((16*10^6) / (50*1024)) - 1 (must be <65536)  //50 Hz
  // turn on CTC mode
  TCCR1B |= (1 << WGM12);
...
void loop(){
  if(Serial.available()>=4) {
    ...
    if(incoming[2] != -1 && incoming[3] != -1)
    {
      ...
      Serial.println(base);
      target_base_step = map(base,0,360,0,400); //change angle to step count
